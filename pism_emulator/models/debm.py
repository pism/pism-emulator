...
class DEBMModel:
    ...
    def __init__(
        ...
        Parameters
        ----------
        air_temp_all_precip_as_rain : float, optional
            Air temperature at which all precipitation falls as rain, by default 275.15
        air_temp_all_precip_as_snow : float, optional
            Air temperature at which all precipitation falls as snow, by default 273.15
        interpret_precip_as_snow : bool, optional
            Whether to interpret all precipitation as snow, by default False
        melting_threshold_temp : float, optional
            Melting threshold temperature, by default 266.65
        positive_threshold_temp : float, optional
            Positive threshold temperature, by default 273.15
        albedo_input_periodic : bool, optional
            Whether the albedo input is periodic, by default False
        albedo_min : float, optional
            Minimum albedo, by default 0.47
        albedo_max : float, optional
            Maximum albedo, by default 0.82
        albedo_slope : float, optional
            Slope of the albedo, by default -790
        c1 : float, optional
            Parameter c1, by default 29.0
        c2 : float, optional
            Parameter c2, by default -93.0
        ice_density : float, optional
            Density of ice, by default 917.0
        ...
        interpolate_rule : str, optional
            Rule for interpolation, by default "linear"
        max_evals_per_year : int, optional
            Maximum number of evaluations per year, by default 52
        paleo_eccentricity : float, optional
            Paleo eccentricity, by default 0.0167
        paleo_enabled : bool, optional
            Whether paleo parameters are enabled, by default False
        paleo_obliquity : float, optional
            Paleo obliquity, by default 23.44
    ...
    def _expand(self, array, shape):
        ...
        if array.shape == shape:
            ...
            return np.broadcast_to(array, (shape[0],) + array.shape)
        elif array.shape == ():
    ...
    def snow_accumulation(
        ...
        ----------
        temperature : np.ndarray
            Near-surface air temperature in degrees Celcius. Shape: (N,).
        precipitation : np.ndarray
            Precipitation rate in meter per year. Shape: (N,).

        Returns
        -------
        np.ndarray
            Snow accumulation in meters. Shape: (N,).

        Raises
        ------
        ValueError
            If temperature and precipitation arrays have different shapes.
        """
...
class TorchDEBMModel:
    ...
    def __call__(
        ...
        *precipitation*: array_like
        ...
        # ensure numpy arrays
        temperature = torch.asarray(temperature)
        precipitation = torch.asarray(precipitation)

        # initialize snow depth and melt rates
        snow_depth = torch.zeros_like(temperature)
        snow_melted = torch.zeros_like(temperature)
        ice_melted = torch.zeros_like(temperature)
        insolation_melt = torch.zeros_like(temperature)
        temperature_melt = torch.zeros_like(temperature)
        offset_melt = torch.zeros_like(temperature)
        total_melt = torch.zeros_like(temperature)
        melt = torch.zeros_like(temperature)
    ...
    def _expand(self, array, shape):
        ...
        if array.shape == shape:
            ...
            return torch.broadcast_to(array, (shape[0],) + array.shape)
        elif array.shape == ():
            return torch.full(shape, array)
        else:
            raise ValueError(
                "could not expand array of shape %s to %s" % (array.shape, shape)
            )
    ...
    def melt(
        ...
        eps = 1.0e-4
        T_eff = torch.where(T_eff < eps, 0.0, T_eff)
        ...
        total_melt = insolation_melt + temperature_melt + offset_melt
        total_melt = torch.maximum(total_melt, torch.tensor(0.0))
    ...
    def CalovGreveIntegrand(
        ...
        >>>    sigma = torch.array([4.2, 2.0])
        >>>    temperature = torch.array([1.0, 2.1])
        ...
        Z = temperature / (np.sqrt(2.0) * sigma)
        b = (sigma / np.sqrt(2.0 * np.pi)) * torch.exp(-Z * Z) + (temperature / 2.0) * (
            1.0 - sp.erf(-Z)
        )
    ...
    def hour_angle(
        self, phi: torch.tensor, latitude: torch.tensor, declination: torch.tensor
    ) -> torch.tensor:
        """
        The hour angle (radians) at which the sun reaches the solar angle `phi`
        ...
        Parameters
        ----------
        phi : float, list of floats or numpy.tensor
            angle (radians)
        latitude : float, list of floats or numpy.tensor
            latitude (radians)
        declination : float, list of floats or numpy.tensor
            solar declination angle (radians)
        ...
        >>>    phi = torch.array([torch.pi / 4])
        >>>    latitude = torch.array([torch.pi / 4])
        >>>    declination = torch.array([torch.pi / 8])
        >>>    hour_angle = debm.hour_angle(phi, latitude, declination)
        >>>    hour_angle
        array([0.839038303283583])
        """
    ...
    def solar_longitude(
        self,
        year_fraction: torch.tensor,
        eccentricity: torch.tensor,
        perihelion_longitude: torch.tensor,
    ) -> torch.tensor:
        """
        Solar longitude (radians) at current time in the year.
        ...
        year_fraction : numpy.tensor
            fraction (1)
        eccentricity : numpy.tensor
            eccentricity of the earth's orbit (1)
        perihelion_longigute : numpy.tensor
            perihelion longitude (radians)

        Returns
        ----------
        solar_longitude: numpy.tensor
            solar longitude at current time in the year (radians)
        ...
        >>>    year_fraction = torch.array([0.5])
        >>>    eccentricity = torch.array([0.1])
        >>>    perihelion_longitude = torch.array([torch.pi/4])
        >>>    solar_longitude = debm.solar_longitude(year_fraction, eccentricity, perihelion_longitude)
        >>>    solar_longitude
        array([2.08753274])
    ...
    def distance_factor_present_day(self, year_fraction: torch.tensor) -> torch.tensor:
        """
        The unit-less factor scaling top of atmosphere insolation according to the Earth's
        distance from the Sun.
        ...
        Returns
        ----------
        distance_factor_present_day: numpy.tensor
            distance factor present day ()
    ...
    def distance_factor_paleo(
        self,
        eccentricity: torch.tensor,
        perhelion_longitude: torch.tensor,
        solar_longitude: torch.tensor,
    ) -> torch.tensor:
        """
        Calculate paleo distance factor
        ...
        eccentricity : numpy.tensor
            eccentricity of the earth’s orbit (no units)
        perhelion_longitude : numpy.tensor
            perihelion longitude (radians)
        solar_longitude : numpy.tensor
            solar longitude (radians)
    ...
    def solar_declination_present_day(self, year_fraction: torch.tensor) -> torch.tensor:
        ...
        Returns
        ----------
        solar_declination: numpy.tensor
            Solar declination present day ()
    ...
    def solar_declination_paleo(
        self, obliquity: torch.tensor, solar_longitude: torch.tensor
    ) -> torch.tensor:
        """
        Solar declination (radians). This is the "paleo" version used when
        the trigonometric expansion (equation 2.2.10 in Liou 2002) is not valid.
        ...
        >>>    obliquity = torch.array([torch.pi / 4])
        >>>    solar_longitude = torch.array([torch.pi * 3 / 4])
        >>>    solar_declination = debm.solar_declination_paleo(obliquity, solar_longitude)
        >>>   solar_declination
        array([0.55536037])
        """
    ...
    def insolation(
        ...
        Parameters
        ----------
        solar_constant : numpy.tensor
            solar constant (W/m^2)
        distance_factor : numpy.tensor
            distance factor (no units)
        hour_angle : numpy.tensor
            hour angle (radians)
        latitude : numpy.tensor
            latitude (radians)
        declination : numpy.tensor
            declination (radians)

        Returns
        ----------
        insolation numpy.tensor
            Solar declination paleo (radians)
        ...
        >>>    solar_constant = torch.array([1361.0])
        >>>    distance_factor = torch.array([1.1])
        >>>    hour_angle = torch.array([0.8])
        >>>    latitude = torch.array([torch.pi / 4])
        >>>    declination = torch.array([torch.pi / 8])
        >>>    insolation = debm.insolation(solar_constant, distance_factor, hour_angle, latitude, declination)
        >>>    insolation
    ...
    def albedo(self, melt_rate: torch.tensor) -> torch.tensor:
        ...
        return torch.maximum(
            self.albedo_max + self.albedo_slope * melt_rate * self.ice_density,
            torch.zeros_like(melt_rate) + self.albedo_min,
        )
    ...
    def atmosphere_transmissivity(self, elevation: torch.tensor) -> torch.tensor:
        ...
        >>>    elevation = torch.array([0.0, 1000.0, 2000.0])
        >>>    transmissivity = debm.atmosphere_transmissivity(elevation)
        >>>    transmissivity
        array([0.65 , 0.682, 0.714])
        """
